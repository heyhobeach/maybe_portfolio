...
public:
	...
	{
		std::cout << "start PostSend" << std::endl;
...
private:
	...
	{
		std::cout << "enter handle_recive" << std::endl;
		if( error )
		{
		...
		else
		{
			...
			while( nPacketData > 0 )
			{
				...
				{
					std::cout << "nPacketData < sizeofe(PACKET_HEADER" << std::endl;
					break;
				}
...
int main()
{
	...
	{
		std::cout << "szInputMessage  " << szInputMessage <<"  length : "<<strnlen_s(szInputMessage, MAX_MESSAGE_LEN) << std::endl;

		if( strnlen_s( szInputMessage, MAX_MESSAGE_LEN ) == 0 )
		{
			size_t len = strnlen_s(szInputMessage, MAX_MESSAGE_LEN);
			std::cout <<"szInputMessage  " << szInputMessage << std::endl;
		...
		{
			std::cout << "로그인 되어있음 채팅 데이터 전송" << std::endl;
